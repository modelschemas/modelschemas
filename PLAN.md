...
3. Verify against the task's acceptance criteria. Run `bun --bun run test` and
   `bun --bun run lint`. For route/API tasks, exercise the endpoint against
...
4. Check the task off (`- [x]`), append a one-line note under it if anything
   ...
   (`feat(api): ...`, `chore(db): ...`).
5. If blocked (missing secret, upstream outage, ambiguous requirement), check the task
   off as `- [x] ~BLOCKED:` with the reason, move on to the next task, and surface the
   blocker in the iteration summary.
6. Stop the loop when every task is checked.
...
- **Two-tier refresh loop** (Cloudflare Cron Triggers):
  - *Fast tier, every 15 min*: poll each provider's cheap model-list endpoint
    (`/v1/models` etc.). New/removed model → update D1, record a change event,
    fire webhooks. This is what catches "new model just dropped".
  - *Slow tier, daily 05:00 UTC*: full OpenAPI spec sync per provider (port of the
...
- [ ] **0.1 Move the database to D1.** Add a `d1_databases` binding (`DB`) to
  ...
  `todos` table. *Accepts:* dev server boots, a trivial query against D1 works locally.
- [ ] **0.2 Custom worker entry with cron support.** Create `src/worker.ts` exporting
  ...
  *Accepts:* `wrangler dev --test-scheduled` + `curl /__scheduled?cron=*/15+*+*+*+*`
...
- [ ] **0.3 KV namespace + helpers.** Add `kv_namespaces` binding (`SCHEMA_CACHE`).
  ...
  *Accepts:* round-trip test passes under vitest (miniflare env via vite plugin).
- [ ] **0.4 Better Auth on D1.** Wire `src/lib/auth.ts` to the drizzle adapter over D1,
  ...
  ['autonomous', 'delegated']`, empty capabilities (filled in Phase 5). *Accepts:*
...
- [ ] **1.1 Core tables** in `src/db/schema.ts` (drizzle, sqlite dialect):
  ...
  *Accepts:* migration generated and applied locally; drizzle types compile.
- [ ] **1.2 Cache + delivery tables:**
  ...
  *Accepts:* migration applies; FK relations defined.
- [ ] **1.3 Seed script.** `scripts/seed.ts` inserting the 7 providers with their spec
  ...
  URL, etc.). Runnable via `bun run seed` against local D1. *Accepts:* `providers`
...
- [ ] **2.1 Provider registry.** `src/server/providers/types.ts` +
  ...
  skipped with a recorded warning when the secret is absent. *Accepts:* unit tests
...
- [ ] **2.2 Schema extraction + bundling.** `src/server/ingest/bundle.ts`: given a
  ...
  stringify → SHA-256 content hash. *Accepts:* unit tests with fixture specs prove
...
- [ ] **2.3 Sync engine.** `src/server/ingest/sync.ts`: per provider —
  ...
  *Accepts:* integration test with a fixture spec: first run inserts, second run
...
- [ ] **2.4 Model poller.** `src/server/ingest/poll-models.ts`: per provider, call
  ...
  `model.removed` / `model.updated` changes, bump `lastSeenAt`. *Accepts:* fixture
...
- [ ] **2.5 Wire crons.** `scheduled` dispatch: 15-min cron → `pollAllModels(env, ctx)`,
  ...
  keep each provider's work sequential to respect subrequest limits. *Accepts:*
...
- [ ] **3.1 `src/server/cache.ts`** — `swr(env, ctx, key, fetcher, { staleTime, hardTtl })`:
  ...
  `cache_meta`. *Accepts:* unit tests for all three paths and the stuck-flag case.
- [ ] **3.2 HTTP cache semantics.** Response helper adding `ETag` (content hash),
  ...
  read endpoints in Phase 4. *Accepts:* test asserts 304 on matching ETag.
...
- [ ] **4.1 Discovery surface.** `GET /v1` (service description + endpoint index),
  ...
  service's own OpenAPI doc, hand-maintained in `src/server/openapi.ts`). *Accepts:*
...
- [ ] **4.2 Catalog endpoints.**
  ...
  *Accepts:* curl each against seeded/synced local data; filters covered by tests.
- [ ] **4.3 Schema endpoints.**
  ...
  All via the SWR layer + ETag helper. *Accepts:* schemas served byte-identical to D1
...
- [ ] **4.4 Validation endpoint.** `POST /v1/validate` with
  ...
  using `@cfworker/json-schema`. *Accepts:* test with one valid + one invalid
  Anthropic messages payload.
- [ ] **4.5 Changes feed.** `GET /v1/changes?since=&provider=&type=` (cursor-paginated)
  — the polling-friendly alternative to webhooks. *Accepts:* cursor pagination test.
...
- [ ] **5.1 Agent Auth provider surface.** Expose the discovery document at
  ...
  *Accepts:* discovery doc renders; `POST /agent/register` → capability request →
...
- [ ] **5.2 JWT verification on owned routes.** For agent-authed access to our native
  ...
  grant check, used by subscriptions (Phase 6) and `/v1/agents/me`. *Accepts:* test
...
- [ ] **5.3 API-key fallback.** `POST /v1/agents/register-key` `{ name, email? }` →
  ...
  protocol. *Accepts:* register → authed call with the key succeeds.
- [ ] **5.4 Rate limiting.** Per-agent/per-key and per-IP (anonymous) limits via
  ...
  60 req/h; authenticated: 5k req/h. 429s include `Retry-After`. *Accepts:* test
  exhausts the anonymous window and sees 429.
- [ ] **5.5 Usage endpoint.** `GET /v1/agents/me` — agent identity, grants, limits,
  current usage. *Accepts:* curl with agent JWT and with API key.
...
- [ ] **6.1 Subscriptions CRUD.** `POST|GET|DELETE /v1/subscriptions` (authed):
  ...
  returned once. *Accepts:* CRUD round-trip test.
- [ ] **6.2 Webhook delivery.** On new `changes` rows, enqueue `webhook_deliveries`;
  ...
  subscription auto-paused + recorded. *Accepts:* integration test with a local
...
- [ ] **7.1 MCP server endpoint** at `/mcp` (streamable HTTP, Workers-native) exposing
  ...
  `recent_changes`. Reuse the Phase 4 service functions directly. *Accepts:* MCP
...
- [ ] **7.2 Minimal human dashboard.** `/` route: provider sync status, model counts,
  ...
  *Accepts:* renders against local data.
- [ ] **7.3 Docs page** `/docs`: quickstart for agents (register → fetch schema →
  validate → subscribe), rendered from the same content as `llms.txt`. *Accepts:*
...
- [ ] **7.4 Generated TS client (`@modelschemas/client`).** Convert the repo to bun
  ...
  spec. *Accepts:* client compiles; smoke test against local dev lists providers and
...
- [ ] **7.5 CLI (`packages/cli`, bin: `modelschemas`).** Thin command layer over the
  ...
  (default when not a TTY) so the CLI is itself agent-friendly. *Accepts:*
...
- [ ] **7.6 Distributable agent skill.** `skill/modelschemas/SKILL.md` (frontmatter:
  ...
  from `/docs` and `llms.txt`. *Accepts:* frontmatter valid; installing the dir into
...
- [ ] **8.1 Production setup.** Create real D1 + KV resources, set IDs in
  ...
  (`FAL_KEY`, `ELEVENLABS_API_KEY`, …), `ADMIN_KEY`. Document in README. *Accepts:*
  `bun run deploy` succeeds; `/v1/status` live.
- [ ] **8.2 First production sync.** Trigger admin sync for all providers; verify
  `/v1/models?activity=chat` returns real multi-provider data in prod. *Accepts:*
...
- [ ] **8.3 Observability.** `wrangler.jsonc` `observability.enabled`, structured
  ...
  README runbook for "a provider sync is failing". *Accepts:* logs visible via
...
- [ ] **8.4 Publish client + CLI.** npm publish `@modelschemas/client` and the
  ...
  README, `/docs`, and the skill. *Accepts:* `bunx modelschemas@latest whoami`
  works against prod.
- [ ] **8.5 Update CLAUDE.md** with the new architecture (D1/KV bindings, cron
  ...
  starter description. *Accepts:* CLAUDE.md matches reality.
